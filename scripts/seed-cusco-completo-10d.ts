
import { db } from "../server/db";
import { destinations, hotels, itineraryDays, inclusions, exclusions } from "../shared/schema";
import { eq, and } from "drizzle-orm";

async function seedCuscoCompleto10D() {
  try {
    console.log("Iniciando carga del programa Tour Cusco Completo + Lima, Paracas, Nazca, Huacachina: 10 Días - 9 Noches...");

    // Limpiar si existe
    await db.delete(destinations).where(
        and(
            eq(destinations.name, "Tour Cusco Completo + Lima, Paracas, Nazca, Huacachina: 10 Días - 9 Noches"),
            eq(destinations.country, "Perú")
        )
    );

    // 1. Insertar Destino
    const [destination] = await db.insert(destinations).values({
      name: "Tour Cusco Completo + Lima, Paracas, Nazca, Huacachina: 10 Días - 9 Noches",
      country: "Perú",
      duration: 10,
      nights: 9,
      description: "El circuito más completo para conocer Perú. Inicia en Lima y recorre la costa sur (Islas Ballestas, Oasis de Huacachina, Viñedos y Sobrevuelo a Líneas de Nazca). Continúa en los Andes con Cusco, Valle Sagrado, Machu Picchu, Laguna Humantay y la Montaña de 7 Colores. Precios válidos hasta el 30 de Noviembre del 2026",
      basePrice: "0",
      category: "internacional",
      isPromotion: false,
      imageUrl: "https://images.unsplash.com/photo-1526392060635-9d6019884377?auto=format&fit=crop&q=80", // Machu Picchu placeholder
    }).returning();

    console.log(`Destino creado con ID: ${destination.id}`);

    // 2. Insertar Hoteles
    const hotelsData = [
      {
        name: "Hotel El Tambo 1 (Miraflores)",
        location: "Lima",
        category: "3 Estrellas",
        nights: 3,
        destinationId: destination.id
      },
      {
        name: "DM Nazca",
        location: "Nazca",
        category: "Turista",
        nights: 1,
        destinationId: destination.id
      },
      {
        name: "Illa Hotel",
        location: "Cusco",
        category: "3 Estrellas",
        nights: 5,
        destinationId: destination.id
      }
    ];
    await db.insert(hotels).values(hotelsData);
    console.log("Hoteles insertados");

    // 3. Insertar Itinerario
    const itineraryData = [
      {
        dayNumber: 1,
        title: "Llegada a Lima",
        description: "Llegada a Lima y traslado al hotel según la hora de llegada de su vuelo.",
        activities: ["Traslado de llegada"],
        meals: [],
        destinationId: destination.id
      },
      {
        dayNumber: 2,
        title: "City Tour Lima",
        description: "Recojo en hotel (aprox. 08:50 AM) para inicio del City Tour. Fin del tour aprox. 01:30 PM.",
        activities: ["City Tour Lima"],
        meals: ["Desayuno"],
        destinationId: destination.id
      },
      {
        dayNumber: 3,
        title: "Islas Ballestas, Huacachina y Traslado a Nazca",
        description: "Recojo de madrugada (04:50 AM). Salida a Paracas. Tour marítimo a Islas Ballestas. Traslado a Ica para visita a viñedo El Nieto. Visita al Oasis de Huacachina con paseo en Buggies y Sandboarding. Traslado a Nazca en movilidad compartida (06:20 PM). Llegada y traslado al hotel.",
        activities: ["Islas Ballestas", "Viñedo El Nieto", "Oasis de Huacachina", "Buggies y Sandboard", "Traslado a Nazca"],
        meals: ["Desayuno"],
        destinationId: destination.id
      },
      {
        dayNumber: 4,
        title: "Sobrevuelo Líneas de Nazca y Retorno a Lima",
        description: "Desayuno. Sobrevuelo a las Líneas de Nazca para observar 12 geoglifos (Colibrí, Mono, Araña, etc.). Traslado a la estación de bus. Viaje en bus Cruz del Sur hacia Lima (12:30 PM). Llegada a Lima (aprox 08:30 PM) y traslado al hotel.",
        activities: ["Sobrevuelo Líneas de Nazca", "Bus Nazca - Lima"],
        meals: ["Desayuno"],
        destinationId: destination.id
      },
      {
        dayNumber: 5,
        title: "Llegada a Cusco y City Tour Arqueológico",
        description: "Traslado al aeropuerto en Lima (06:45 AM). Vuelo a Cusco. Llegada y traslado al hotel. Por la tarde (02:00 PM), inicio del City Tour Arqueológico visitando Sacsayhuaman, Kenko, Puka Pukará y Tambo Machay.",
        activities: ["Traslados aeropuerto", "City Tour Arqueológico", "Sacsayhuaman", "Tambo Machay"],
        meals: ["Desayuno"],
        destinationId: destination.id
      },
      {
        dayNumber: 6,
        title: "Valle Sagrado: Maras, Moray y Pisac",
        description: "Recojo 06:40 AM. Visita a Chinchero, Salineras de Maras y andenes de Moray. Almuerzo incluido. Visita a Ollantaytambo y Pisaq. Retorno a Cusco aprox. 07:00 PM.",
        activities: ["Chinchero", "Salineras de Maras", "Moray", "Ollantaytambo", "Pisaq"],
        meals: ["Desayuno", "Almuerzo"],
        destinationId: destination.id
      },
      {
        dayNumber: 7,
        title: "Tour a Machu Picchu",
        description: "Recojo 03:30 AM. Tren desde Ollantaytambo hacia Aguas Calientes. Tour guiado en Machu Picchu (aprox 09:30 AM). Retorno en tren a Ollantaytambo y traslado a Cusco, llegando aprox 07:30 PM.",
        activities: ["Tren a Machu Picchu", "Visita guiada Machu Picchu"],
        meals: ["Desayuno"],
        destinationId: destination.id
      },
      {
        dayNumber: 8,
        title: "Laguna Humantay",
        description: "Recojo 04:30 AM. Traslado a Mollepata e inicio del trekking a la Laguna Humantay. Almuerzo incluido (según inclusiones). Retorno a Cusco aprox. 05:00 PM.",
        activities: ["Trekking Laguna Humantay"],
        meals: ["Desayuno", "Almuerzo"],
        destinationId: destination.id
      },
      {
        dayNumber: 9,
        title: "Montaña de 7 Colores (Vinicunca)",
        description: "Recojo 04:30 AM. Caminata hacia Vinicunca. Almuerzo incluido. Retorno a Cusco aprox. 05:30 PM.",
        activities: ["Trek Montaña 7 Colores"],
        meals: ["Desayuno", "Almuerzo"],
        destinationId: destination.id
      },
      {
        dayNumber: 10,
        title: "Vuelo de Retorno",
        description: "Desayuno. Partida hacia el aeropuerto según hora de su vuelo para retorno.",
        activities: ["Traslado de salida"],
        meals: ["Desayuno"],
        destinationId: destination.id
      }
    ];
    await db.insert(itineraryDays).values(itineraryData);
    console.log("Itinerario insertado");

    // 4. Insertar Inclusiones
    const inclusionsData = [
      "Traslados Aeropuerto/hotel/Aeropuerto en Lima y Cusco",
      "3 noches de hospedaje en Lima (Hotel El Tambo 1)",
      "1 noche de hospedaje en Nazca (DM Nazca)",
      "6 noches de hospedaje en Cusco (Illa Hotel)",
      "Todos los desayunos (Box Breakfast para día Machu Picchu)",
      "City Tour en Lima",
      "Excursión Islas Ballestas y Balneario de Paracas",
      "Visita a Vitivinícola en Ica",
      "Paseo en Buggies y Sandboard en Huacachina",
      "Sobrevuelo a las Líneas de Nazca (con impuestos aeroportuarios)",
      "Traslado turístico Lima - Ica y Ica - Nazca (Pool)",
      "Tickets de bus Cruz del Sur Nazca - Lima",
      "City tour Arqueológico en Cusco",
      "Tour Valle Sagrado, Maras y Moray con almuerzo",
      "Tour a Machu Picchu (Ingreso y Guía)",
      "Tour a Laguna Humantay con desayuno y almuerzo",
      "Tour a Vinicunca con desayuno y almuerzo",
      "Ticket de Tren Voyager o Expedition",
      "Boleto Turístico General (BTG)",
      "Boleto de bus Consetur subida y bajada",
      "Guía profesional bilingüe"
    ].map(item => ({
      item: item,
      destinationId: destination.id
    }));
    await db.insert(inclusions).values(inclusionsData);
    console.log("Inclusiones insertadas");

    // 5. Insertar Exclusiones
    const exclusionsData = [
      "Boletos aéreos Lima - Cusco - Lima",
      "Almuerzos (excepto los indicados en tours específicos)",
      "Cenas",
      "Comisión por transferencia de dinero"
    ].map(item => ({
      item: item,
      destinationId: destination.id
    }));
    await db.insert(exclusions).values(exclusionsData);
    console.log("Exclusiones insertadas");

    console.log("¡Programa Tour Cusco Completo + Lima, Paracas, Nazca, Huacachina: 10 Días - 9 Noches cargado exitosamente!");
    process.exit(0);
  } catch (error) {
    console.error("Error al cargar el programa:", error);
    process.exit(1);
  }
}

seedCuscoCompleto10D();
