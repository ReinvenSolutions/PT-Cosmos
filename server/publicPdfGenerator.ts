...
export async function generatePublicQuotePDF(
  ...
  const titleY = 95;
  const passengers = 1; // Always use 1 passenger for pricing display
  ...
  doc.text(titleText, leftMargin, titleY, {
  ...
  doc.font("Helvetica-Bold").fontSize(11).fillColor(textColor);
  doc.text(
    `PLAN ${totalDuration} DÍAS - ${totalNights} NOCHES`,
    leftMargin,
    durationY,
  );
  ...
  doc.text(`creado ${currentDate}`, 0, mainImageY - 15, {
  ...
  doc.text(
    "P R E S U P U E S T O    P A R A    S U    V I A J E",
    leftMargin,
    budgetY,
  );
  ...
  const isTurkeyEsencial = data.destinations.some(
    (d) =>
  ...
  const isTurkey = data.destinations.some(
    (d) =>
  ...
  cityStops.forEach((stop, index) => {
    ...
    doc
      .roundedRect(leftMargin, currentY, numberBoxSize, numberBoxSize, 3)
      .fillAndStroke(primaryColor, primaryColor);

    doc.font("Helvetica-Bold").fontSize(16).fillColor("white");
    doc.text(stop.number.toString(), leftMargin, currentY + 12, {
      width: numberBoxSize,
      align: "center",
    });
    ...
    doc.text(
      stop.nights.toString(),
      pageWidth - rightMargin - 60,
      currentY + 5,
      {
        width: 30,
        align: "right",
  ...
  if (
    ...
    data.outboundFlightImages.length > 0
  ) {
    console.log(
      "[PDF Generator] Outbound flight images:",
      data.outboundFlightImages,
    );
    doc.addPage();
    addPageBackground();
    ...
    for (let index = 0; index < data.outboundFlightImages.length; index++) {
      ...
      console.log(
        `[PDF Generator] Processing outbound image ${index}:`,
        imageUrl,
      );
        ...
        try {
          ...
          if (fs.existsSync(fullPath)) {
            try {
              ...
              const isLastImage =
                index === data.outboundFlightImages.length - 1;
              ...
              if (isLastImage) {
                ...
                console.log(
                  `[PDF Generator] Maximizing outbound image ${index} to ${Math.round(imageHeight)}px (available: ${Math.round(maxAvailableHeight)}px, natural: ${Math.round(naturalHeight)}px, currentY: ${Math.round(flightImageY)})`,
                );
              } else {
                ...
                if (flightImageY + imageHeight + 20 > pageHeight - 50) {
                  ...
                  const maxHeightOnNewPage =
                    pageHeight - 50 - flightImageY - 40;
  ...
  if (allHotels.length > 0) {
    ...
    Object.entries(hotelGroups).forEach(([location, hotels]) => {
      ...
      doc.text(`${location.toUpperCase()}:`, leftMargin, doc.y, {
        continued: true,
      });
      ...
      if (location === "Kusadasi/Esmirna") {
        ...
        doc.text(
          " (Posible mejora en el Hotel Le Bleu sujeta a disponibilidad)",
          { width: contentWidth },
        );
      }
  ...
  if (
    ...
    data.returnFlightImages.length > 0
  ) {
    console.log(
      "[PDF Generator] Return flight images:",
      data.returnFlightImages,
    );
    doc.addPage();
    addPageBackground();
    ...
    doc
      ...
      .fontSize(18)
      .fillColor(textColor)
      .text("VUELO REGRESO", leftMargin, 80, {
        align: "center",
        width: contentWidth,
      });
    ...
    for (let index = 0; index < data.returnFlightImages.length; index++) {
      ...
      console.log(
        `[PDF Generator] Processing return image ${index}:`,
        imageUrl,
      );
      // Extract filename from URL (format: /api/images/filename.ext)
      const filename = imageUrl.split("/").pop();
          ...
          if (fs.existsSync(fullPath)) {
            try {
              ...
              if (isLastImage) {
                ...
                imageHeight = Math.min(naturalHeight, maxAvailableHeight);
                console.log(
                  `[PDF Generator] Maximizing return image ${index} to ${Math.round(imageHeight)}px (available: ${Math.round(maxAvailableHeight)}px, natural: ${Math.round(naturalHeight)}px, currentY: ${Math.round(flightImageY)})`,
                );
              } else {
                // For non-last images, check if they fit
                if (flightImageY + imageHeight + 20 > pageHeight - 50) {
  ...
  doc.font("Helvetica-Bold").fontSize(18).fillColor(textColor);
  doc.text("ASISTENCIA MEDICA PARA TU VIAJE", leftMargin, topMargin, {
    align: "center",
    width: contentWidth,
  });
  ...
  if (hasTurkeyDestinations) {
    ...
    doc.text("TOUR OPCIONALES", leftMargin, headerY + 10, {
      align: "center",
      width: contentWidth,
    });
    ...
    const addTableRow = (name: string, price: string) => {
      ...
      doc
        .rect(leftMargin + nameColWidth, currentY, priceColWidth, rowHeight)
        .stroke("#3b82f6");
    ...
    const tours = [
      {
        name: "Paseo en Globo (de 15/Mar 2026 a 31/Oct 2026)",
        price: "415 USD",
      },
      {
        name: "Paseo en Globo (de 01/Nov 2026 a 14/Mar 2027)",
        price: "384 USD",
      },
      ...
      {
        name: "Noche Turca en   7F Ö ui (en barco con cena show)",
        price: "154 USD",
      },
      ...
      {
        name: "6 almuerzos (en las ciudades del itinerario, menos en Estambul)",
        price: "185 USD",
      },
    ...
    // Dibujar header azul
    doc
      .rect(leftMargin, combo1HeaderY, contentWidth, 20)
      .fillAndStroke("#1e40af", "#1e40af");
    // Agregar título "Combo 1" en el header
    doc.fillColor("#ffffff").font("Helvetica-Bold").fontSize(10);
    doc.text("Combo 1", leftMargin, combo1HeaderY + 6, {
      width: contentWidth,
      align: "center",
    });
    ...
    combo1Items.forEach((item, index) => {
      doc
        .rect(leftMargin, currentY, itemsWidth, rowHeight)
        .fillAndStroke(tableRowBg, "#3b82f6");

      doc.fillColor(tableTextColor).font("Helvetica").fontSize(7);
      doc.text(item, leftMargin + 3, currentY + 3, {
        width: itemsWidth - 6,
        continued: false,
      });
    ...
    doc
      .rect(combo1PriceX, combo1GridStartY, priceBoxWidth, combo1TotalHeight)
      .fillAndStroke("#1e40af", "#1e40af");

    doc.fillColor("#ffffff").font("Helvetica-Bold").fontSize(18);
    const priceTextY = combo1GridStartY + combo1TotalHeight / 2 - 9;
    doc.text("1,020 USD", combo1PriceX, priceTextY, {
      width: priceBoxWidth,
      align: "center",
    });
    ...
    // Dibujar header azul
    doc
      .rect(leftMargin, combo2HeaderY, contentWidth, 20)
      .fillAndStroke("#1e40af", "#1e40af");
    // Agregar título "Combo 2" en el header
    doc.fillColor("#ffffff").font("Helvetica-Bold").fontSize(10);
    doc.text("Combo 2", leftMargin, combo2HeaderY + 6, {
      width: contentWidth,
      align: "center",
    });
    ...
    combo2Items.forEach((item, index) => {
      doc
        .rect(leftMargin, currentY2, itemsWidth, rowHeight)
        .fillAndStroke(tableRowBg, "#3b82f6");

      doc.fillColor(tableTextColor).font("Helvetica").fontSize(7);
      doc.text(item, leftMargin + 3, currentY2 + 3, {
        width: itemsWidth - 6,
        continued: false,
      });
    ...
    doc
      .rect(combo2PriceX, combo2GridStartY, priceBoxWidth, combo2TotalHeight)
      .fillAndStroke("#1e40af", "#1e40af");

    doc.fillColor("#ffffff").font("Helvetica-Bold").fontSize(18);
    const priceText2Y = combo2GridStartY + combo2TotalHeight / 2 - 9;
    doc.text("660 USD", combo2PriceX, priceText2Y, {
      width: priceBoxWidth,
      align: "center",
    });
    ...
    console.log(
      "[PDF Generator] Turkey optional tours table added successfully (compact version with combos)",
    );
  }
  ...
  if (isTurkeyEsencial) {
    ...
    doc.font("Helvetica-Bold").fontSize(14).fillColor(primaryColor);
    doc.text("DÍAS FESTIVOS 2026 - TURQUÍA", leftMargin, doc.y, {
      align: "center",
      width: contentWidth,
    });
    doc.moveDown(2);
    ...
    const holidays = [
      ...
      },
      {
        date: "30 marzo",
        description:
          "1º día del feriado religioso, no se harán visitas (bazares y algunos museos cerrados).",
      },
      {
        date: "31 marzo - 01 abril",
        description:
          "Feriado religioso, se realizan visitas pero el gran bazar y bazar de las especias se encuentran cerrados.",
      },
      {
        date: "23 de abril",
        description:
          "Feriado nacional, puede haber cambios en el orden de las visitas. Los bazares están cerrados.",
      },
      {
        date: "01 de mayo",
      ...
      },
      {
        date: "06 de mayo",
        description:
          "Feriado nacional, solo puede haber cambios en el orden de las visitas. Los bazares están cerrados.",
      },
      {
        date: "06 junio",
        description:
